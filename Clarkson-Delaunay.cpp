...
It is called:
     ...
     unsigned int *BuildTriangleIndexList ( void *, float, int *, int , int , int * );

It takes 6 arguments:

unsigned int *BuildTriangleIndexList (
...
integers instead, you only need to do a search and replace to change unsigned int to int.
The few references to unsigned int are from me, not from Clarkson.

The function does not need to be initialized or closed down.
You are likely to want 4 variables to use it:

  unsigned int *triangleIndexList;   <- OUTPUT, this does not need initialization
...
and in the statement that draws the triangles. For example:
  ...
  bd.ByteWidth = sizeof( unsigned int ) * numTriangleVertices;    <-- numTriangleVertices from the function
...
static unsigned int *ptrToOutputList ;
...
if (numPointsProcessed >= totalInputPoints)  {
   ...
   ptrToOutputList = (unsigned int*)malloc((maxOutputEntries + 1) * sizeof(unsigned int));
   currenOutputIndex = 0;
   return 0;
}
   ...
   if (v0 >= 0 && v1 >= 0 && v2 >= 0 && v3 >= 0)  {
      ...
      if (triangleDirection)  {
         ...
         if ( ((triangleDirection > 0) && !isCW)  ||   // if user wants CW triangles, but it is not CW
            ...
            ptrToOutputList[currenOutputIndex++] = (unsigned int)v2;
            ptrToOutputList[currenOutputIndex++] = (unsigned int)v1;
            ptrToOutputList[currenOutputIndex++] = (unsigned int)v0;
            return;
         }
      }
      ptrToOutputList[currenOutputIndex++] = (unsigned int)v0;
